...
public final class DeliveryTransactionProxyFactory {
	private DeliveryTransactionProxyFactory() {}

	public static <T> T readWrite(Class<T> type, T target, PlatformTransactionManager manager) {
		return proxy(type, target, manager, false, TransactionDefinition.PROPAGATION_REQUIRED);
	}

	public static <T> T readOnly(Class<T> type, T target, PlatformTransactionManager manager) {
		return proxy(type, target, manager, true, TransactionDefinition.PROPAGATION_REQUIRED);
	}

	public static <T> T requiresNew(Class<T> type, T target, PlatformTransactionManager manager) {
		return proxy(type, target, manager, false, TransactionDefinition.PROPAGATION_REQUIRES_NEW);
	}

	private static <T> T proxy(Class<T> type, T target, PlatformTransactionManager manager,
